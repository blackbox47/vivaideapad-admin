  ...
  return (
    ...
    >
      ...
      <ScrollArea className="min-h-0 flex-1">
        <nav className="flex flex-col pr-1" aria-label="Contributor space">
          {CREATOR_NAV_ITEMS.map((item) => {
            ...
            return (
              <NavLink
                key={item.href}
                to={item.href}
                end={item.href === CREATOR_ROUTES.dashboard}
                onClick={close}
                className={({ isActive }) =>
                  cn(
        ...
        {isLoading || !user ? (
          ...
          <>
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-[11px] text-sidebar-muted">Contributor</p>
            </div>
          </>
        )}
      </div>
