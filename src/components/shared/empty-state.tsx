...
export function EmptyIllustration({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) {
  ...
  const bubbleClass = isSm
    ? 'absolute -top-1 -right-1 bg-primary text-primary-foreground rounded-full px-1.5 py-0.5 flex items-center gap-0.5 shadow-sm'
    : 'absolute -top-1 -right-2 bg-primary text-primary-foreground rounded-full px-2 py-0.5 flex items-center gap-1 shadow-sm';

  const dotClass = isSm
    ? 'w-1 h-1 rounded-full bg-white opacity-90 inline-block'
    : 'w-1.5 h-1.5 rounded-full bg-white opacity-90 inline-block';

  const rectDotClass = isSm
    ? 'w-1 h-1 rounded-[0.5px] bg-white opacity-90 inline-block'
    : 'w-1.5 h-1.5 rounded-[1px] bg-white opacity-90 inline-block';

  const triClass = isSm
    ? 'w-0 h-0 border-l-[2px] border-l-transparent border-r-[2px] border-r-transparent border-b-[3.5px] border-b-white opacity-90 inline-block'
    : 'w-0 h-0 border-l-[3px] border-l-transparent border-r-[3px] border-r-transparent border-b-[5px] border-b-white opacity-90 inline-block';
  ...
  return (
    ...
    >
      <svg
        className={cn(svgClass, 'text-border-subtle')}
        fill="none"
        viewBox="0 0 80 80"
