...
export default function SubmitIdeaOverview() {
  ...
  const activeTopicId =
    selectedTopicId ??
    submissionData?.conceptId ??
    (topicIdFromUrl || undefined);
    ...
    return (
      <Card className="max-w-lg">
        <CardHeader>
          ...
          <CardDescription>
            {getApiErrorMessage(submissionError) ?? 'Unexpected error'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button
  ...
  const waitingForSubmission =
    isEditing && (isLoadingSubmission || !submissionData);
    ...
    <div className="mx-auto w-full max-w-[1200px] py-4 sm:py-8">
      <section className="mb-10" data-purpose="header-section">
        <h1 className="mb-3 text-4xl sm:text-5xl font-extrabold tracking-tight leading-tight text-foreground">
          {isEditing ? 'Edit your idea' : 'Submit a new idea'}
        </h1>
      ...
      >
        {waitingForSubmission ? (
          <div className="space-y-4" aria-busy="true" aria-label="Loading submission">
          ...
          <SubmitIdeaForm
            key={
              submissionData
                ? `${submissionData.id}:${submissionData.updatedAt ?? ''}`
                : 'new-idea'
            }
            topics={topics}
